...
后续策略已修正：`nasa_like_wall*` 石头只作为 held-out 测试/诊断数据，不再作为训练采样来源。本报告中的 NASA-like 批次保留为测试/失败诊断记录，不进入训练集。训练主分布已切换到 `convex_poly_wall_train` 和后续 `convex_poly_diverse_train` 等通用凸多面体 profile。
...
- 保留 `nasa_like_wall_v3` 作为 held-out 测试/诊断数据，不进入训练集。
