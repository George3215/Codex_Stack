...
def main() -> int:
    ...
    metrics = {
        ...
        "input_detail": {
            "map_input": "local support observation tensor with depth/valid/target/candidate/gravity/course channels",
            "map_shape": list(bundle.maps.shape[1:]),
            "numeric_input": "pre-simulation candidate pose, target, rock geometry, present-mask, and optional PointNet embedding",
            "numeric_dim": int(bundle.numeric.shape[1]),
            "output": "one scalar ranking score per candidate pose inside a groupwise softmax choice set",
        },
        "output_dim": 1,
        "parameter_count": int(sum(parameter.numel() for parameter in model.parameters())),
        "architecture_detail": {
            "name": "SupportMapCNNRanker",
            "stage": "wall-state observation ranker; replaces part of pose-search scoring using depth/support maps",
            "map_encoder": "Conv2d(C,32,k5)-BN-SiLU-Pool -> Conv2d(32,64,k3)-BN-SiLU-Pool -> Conv2d(64,128,k3)-BN-SiLU-AdaptiveAvgPool",
            "numeric_encoder": "Linear(numeric_dim,64)-LayerNorm-SiLU-Dropout",
            "fusion_head": "Linear(128+64,hidden)-LayerNorm-SiLU-Dropout-Linear(hidden,1)",
            "hidden": args.hidden,
            "dropout": args.dropout,
        },
